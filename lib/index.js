...
function getCache() {
  var cache;
  try {
    cache = JSON.parse(fs.readFileSync('.awspublish', 'utf8'));
  } catch (err) {
    cache = {};
  }
  return cache;
...
/**
 * create a cache file for each s3 file that goes through the stream
 * for each file we store in the cache the key value pair
 * s3.path, s3.etag
 *
 * @see https://github.com/gulpjs/gulp/blob/master/docs/writing-a-plugin/README.md
 * @param  {Object} param
...
module.exports.cache = function() {
  ...
  var stream = through.obj(function (file, enc, cb) {
    if (file.s3
     && file.s3.path
     && file.s3.etag) cache[file.s3.path] = file.s3.etag;

    cb(null, file);
  });

  stream.on('finish', function() {
    fs.writeFileSync('.awspublish', JSON.stringify(cache));
  });
